...
class MemoryQueries {
  ...
  // Projects are stored as a JSON array in projects_json — use LIKE for a
  // substring match, then parse for an exact element check.
  getLastSessionForProject(project) {
    const candidates = this._db.db.prepare(
      "SELECT * FROM sessions WHERE projects_json LIKE ? ORDER BY start_time DESC LIMIT 20"
    ).all(`%${project}%`);
    for (const row of candidates) {
      try {
        const projects = JSON.parse(row.projects_json || '[]');
        if (projects.includes(project)) return row;
  ...
  getRecentSessionsForProject(project, limit) {
    ...
    const candidates = this._db.db.prepare(
      `SELECT * FROM sessions WHERE projects_json LIKE ?
       ORDER BY COALESCE(end_time, start_time) DESC LIMIT ?`
    ).all(`%${project}%`, lim * 4);
    ...
    for (const row of candidates) {
      try {
        const projects = JSON.parse(row.projects_json || '[]');
        if (projects.includes(project)) matched.push(row);
      } catch { /* malformed json — skip */ }
  ...
  resolveAlias(alias) {
    const row = this._db.db.prepare(
      'SELECT project, file FROM aliases WHERE alias = ? LIMIT 1'
    ).get(alias);
    return row || null;
  }
