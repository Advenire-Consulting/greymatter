...
function buildPreamble(planPath, chunkNumber, planContents) {
  ...
  const parts = [
    ...
    '  - Line numbers in "Read first" may have shifted if prior chunks modified these files. Run `node greymatter/scripts/query.js --structure <file> --project <name>` to get current function/definition line numbers before reading. Also available: `--body <file> <function> --project <name>` (extract function source) and `--section <file> script|template|style --project <name>` (extract Svelte sections).',
...
// Assemble the chunk assignment. When `preamble` is true, output matches the
// Sonnet-assignment format (preamble + --- + header + --- + [observations
// + ---] + body). When false (default), output is header + [observations]
// + body separated by blank lines, no preamble, no --- separators.
function assembleAssignment({ planPath, planContents, chunkNumber, observations = '', preamble = false }) {
  ...
  const parts = [];
  if (preamble) {
    parts.push(buildPreamble(planPath, chunkNumber, planContents));
    parts.push('---');
  }
  parts.push(header);
  if (preamble) parts.push('---');
  if (prior) {
...
function computeDispatchPayloads({ planPath, planContents, observations = '', preamble = false }) {
  const path = require('path');
