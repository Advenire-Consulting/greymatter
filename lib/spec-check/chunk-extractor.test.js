...
// --- preamble: false (new default) ---

test('assembleAssignment with preamble:false omits preamble and --- separators', () => {
  const out = assembleAssignment({
    planPath: '/fake/plan.md',
    planContents: SAMPLE_PLAN,
    chunkNumber: 1,
  ...
  assert.doesNotMatch(out, /Sonnet assignment/);
  assert.doesNotMatch(out, /Standing rules/);
  assert.doesNotMatch(out, /Do NOT restart services/);
  assert.doesNotMatch(out, /^---$/m);
  assert.match(out.trimStart(), /^# Plan Title/);
  assert.match(out, /Body of chunk 1/);
});

test('assembleAssignment with preamble:false includes observations block between header and body', () => {
  const out = assembleAssignment({
    planPath: '/fake/plan.md',
    planContents: SAMPLE_PLAN,
    chunkNumber: 2,
  ...
  assert.match(out, /Prior chunk note/);
  assert.doesNotMatch(out, /^---$/m);
  const headerIdx = out.indexOf('# Plan Title');
  const obsIdx = out.indexOf('## Prior agent observations');
  const bodyIdx = out.indexOf('Body of chunk 2');
  assert.ok(headerIdx < obsIdx, 'header must come before observations');
  assert.ok(obsIdx < bodyIdx, 'observations must come before body');
});

test('assembleAssignment defaults preamble to false when option omitted', () => {
  const out = assembleAssignment({
    planPath: '/fake/plan.md',
    planContents: SAMPLE_PLAN,
    chunkNumber: 1,
...
test('computeDispatchPayloads with preamble:false returns body-only payloads', () => {
  const payloads = computeDispatchPayloads({
    planPath: '/fake/dir/plan.md',
    planContents: SAMPLE_PLAN,
    preamble: false,
  ...
  for (const p of payloads) {
    assert.doesNotMatch(p.content, /Sonnet assignment/);
    assert.doesNotMatch(p.content, /Standing rules/);
    assert.doesNotMatch(p.content, /^---$/m);
  }
  assert.match(payloads[0].content.trimStart(), /^# Plan Title/);
});

test('computeDispatchPayloads with preamble:true matches current full output', () => {
  const payloads = computeDispatchPayloads({
    planPath: '/fake/dir/plan.md',
    planContents: SAMPLE_PLAN,
    preamble: true,
  });
  assert.match(payloads[0].content, /Sonnet assignment — Chunk 1/);
  assert.match(payloads[0].content, /Standing rules/);
});
