...
test('spec without spec_section is valid (line refs are plan-only)', () => {
  const specNoLineRef = VALID_SPEC.replace('      spec_section: L10-L50\n', '');
  const r = parseFrontmatter(specNoLineRef, 'test.md');
  assert.equal(r.ok, true, JSON.stringify(r.errors));
});

test('plan without spec_section is invalid (line refs required for plans)', () => {
  const planNoLineRef = VALID_SPEC
    .replace('doc_type: spec', 'doc_type: plan')
    .replace('feature_area: features/x', 'feature_area: features/x\nimplements: 2026-04-07-some-spec')
    .replace('      spec_section: L10-L50\n', '');
  const r = parseFrontmatter(planNoLineRef, 'test.md');
  assert.equal(r.ok, false);
  assert.ok(r.errors.some(e => e.code === 'MISSING_ITEM_FIELD' && e.message.includes('spec_section')));
});
