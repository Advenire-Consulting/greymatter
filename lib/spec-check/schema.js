  ...
  requiredArrays: {
    'touches.files': {
      itemShape: {
        ...
        // Line refs are required only for plans. Specs may include them, but
        // the body churns too much during drafting to keep them in sync —
        // the plan, written from a stable spec, is where they land.
        spec_section: { type: 'line_ref', requiredFor: ['plan'] },
...
function renderTemplate(docType) {
  ...
  const filesEntry = isPlan
    ? `    - path: path/to/file.js             # project-relative, no leading slash
      mode: modify                      # create | modify | delete
      spec_section: L120-L160           # required for plans — where in the SPEC the rationale lives (format: L<start>-L<end>)
    ...
    : `    - path: path/to/file.js             # project-relative, no leading slash
      mode: modify                      # create | modify | delete
      # spec_section / source_lines are NOT required while drafting a spec.
      # The plan, written from the stable spec, is where line refs land.`;

  const schemaExample = isPlan
    ? `    # - table: your_table
    #   change: add_columns             # create | add_columns | add_indexes | drop_column | drop_table | modify_constraint
    #   spec_section: L200-L215         # required for plans`
    : `    # - table: your_table
    #   change: add_columns             # create | add_columns | add_indexes | drop_column | drop_table | modify_constraint`;

  const emitsExample = isPlan
    ? `      # - name: your.event.name
      #   from_file: path/to/emitter.js
      #   spec_section: L250            # required for plans`
    : `      # - name: your.event.name
      #   from_file: path/to/emitter.js`;

  const subscribesExample = isPlan
    ? `      # - name: other.event.name
      #   spec_section: L270            # required for plans`
    : `      # - name: other.event.name`;
